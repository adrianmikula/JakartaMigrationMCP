...
public class McpToolRegistry {
        ...
        public static List<McpToolDefinition> getAllTools() {
                ...
                tools.add(createRunAdvancedScanTool());
                tools.add(createApplyJakartaRecipeTool());
        ...
        private static McpToolDefinition createUpdateDependencyTool() {
                ...
                McpToolDefinition.InputSchema inputSchema = new McpToolDefinition.InputSchema(properties,
                                Arrays.asList("projectPath", "groupId", "artifactId", "currentVersion",
                                                "recommendedVersion"));
        ...
        /**
         * Tool: runAdvancedScan
         * Runs a specialized scan for a specific technology.
         */
        private static McpToolDefinition createRunAdvancedScanTool() {
                Map<String, McpToolDefinition.PropertySchema> properties = new LinkedHashMap<>();
                properties.put("projectPath", McpToolDefinition.PropertySchema.string(
                                "Absolute path to the project root"));
                properties.put("technology", McpToolDefinition.PropertySchema.string(
                                "Technology to scan: 'jpa', 'validation', 'servlet', 'cdi', 'rest', 'soap', 'security', 'build', 'all'"));
                properties.put("includeSource", McpToolDefinition.PropertySchema.booleanSchema(
                                "Whether to scan source code"));
                properties.put("includeBinary", McpToolDefinition.PropertySchema.booleanSchema(
                                "Whether to scan binary artifacts"));

                McpToolDefinition.InputSchema inputSchema = new McpToolDefinition.InputSchema(properties,
                                Arrays.asList("projectPath", "technology"));

                return new McpToolDefinition(
                                "runAdvancedScan",
                                "Executes a deep, technology-specific scan for Jakarta EE migration readiness. " +
                                                "Identifies fine-grained usage of specific Java EE APIs and provides " +
                                                "targeted migration advice for the selected technology stack.",
                                inputSchema,
                                SERVER_NAME,
                                SERVER_VERSION);
        }
        ...
        /**
         * Tool: applyJakartaRecipe
         * Applies a specific Jakarta migration recipe.
         */
        private static McpToolDefinition createApplyJakartaRecipeTool() {
                Map<String, McpToolDefinition.PropertySchema> properties = new LinkedHashMap<>();
                properties.put("projectPath", McpToolDefinition.PropertySchema.string(
                                "Absolute path to the project root"));
                properties.put("recipeName", McpToolDefinition.PropertySchema.string(
                                "Name of the recipe to apply: e.g., 'MigrateJPA', 'MigrateServlet', 'UpdateWebXml'"));
                properties.put("dryRun", McpToolDefinition.PropertySchema.booleanSchema(
                                "If true, only preview changes"));

                McpToolDefinition.InputSchema inputSchema = new McpToolDefinition.InputSchema(properties,
                                Arrays.asList("projectPath", "recipeName"));

                return new McpToolDefinition(
                                "applyJakartaRecipe",
                                "Applies a targeted Jakarta migration recipe to the project. " +
                                                "Unlike the general refactoring tool, this targets specific technologies "
                                                +
                                                "with optimized recipes for complex migration scenarios.",
                                inputSchema,
                                SERVER_NAME,
                                SERVER_VERSION);
        }
