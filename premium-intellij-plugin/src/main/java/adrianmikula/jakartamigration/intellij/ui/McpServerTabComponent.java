...
public class McpServerTabComponent {
    ...
    private JPanel createToolsPanel() {
        ...
        // Get actual tool count from MCP server provider
        JakartaMcpServerProvider provider = JakartaMcpRegistrationActivity.getServerProvider();
        int toolCount = 0;
        if (provider != null && provider.isReady()) {
            toolCount = provider.getToolCount();
        }
        
        // Get tool list from provider if available, otherwise use hardcoded list
        String[] tools;
        if (toolCount > 0) {
            try {
                tools = provider.getTools().stream()
                        .map(McpToolDefinition::getName)
                        .toArray(String[]::new);
            } catch (Exception e) {
                LOG.warn("Could not get tools from provider: " + e.getMessage());
                tools = new String[] {"Error loading tools"};
            }
        } else {
            // Fallback to actual available tools from McpToolRegistry
            tools = new String[] {
                    "analyzeJakartaReadiness - Analyze project readiness for Jakarta EE migration",
                    "analyzeMigrationImpact - Detailed analysis of migration impact and effort",
                    "detectBlockers - Identify migration blockers and critical issues",
                    "recommendVersions - Recommend compatible Jakarta EE dependency versions",
                    "applyOpenRewriteRefactoring - Apply OpenRewrite recipes for automatic migration",
                    "scanBinaryDependency - Scan JAR files for Jakarta EE compatibility",
                    "updateDependency - Update a dependency to a Jakarta-compatible version",
                    "generateMigrationPlan - Generate a phased migration plan",
                    "validateMigration - Validate migration results and completion status",
                    "runAdvancedScan - Run technology-specific deep scans",
                    "applyJakartaRecipe - Apply targeted migration recipes"
            };
        }
