...
> Tools: `CommunityMigrationTools.java`, `JakartaMigrationTools.java`, `SentinelTools.java`
...
| 4 | **Compare migration strategies** | Synthesize a side-by-side comparison of Big Bang, Incremental, and Dependencies-First approaches using existing analysis data. |

After each action, return to this menu so the user can run another option or exit.

## 3. Available MCP Tools

The community MCP server exposes these 6 tools:
...
> **Note:** Premium-only tools (`createReport`, `applyRefactorRecipe`, `listRefactorRecipes`, `getRefactorHistory`, `undoRefactorRecipe`) are **not available** in the community edition and should not be referenced.
