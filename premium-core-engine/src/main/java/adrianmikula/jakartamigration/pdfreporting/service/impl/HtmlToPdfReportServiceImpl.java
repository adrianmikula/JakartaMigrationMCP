...
import adrianmikula.jakartamigration.pdfreporting.service.PdfReportService.RiskAnalysisReportRequest;
import adrianmikula.jakartamigration.dependencyanalysis.domain.DependencyGraph;
import adrianmikula.jakartamigration.risk.RiskScoringService;
import adrianmikula.jakartamigration.pdfreporting.snippet.HtmlSnippet;
...
public class HtmlToPdfReportServiceImpl implements PdfReportService {
    ...
    private String generateRiskAnalysisHtmlWithSnippets(RiskAnalysisReportRequest request) {
        ...
        try {
            ...
            List<HtmlSnippet> snippets = factory.createSnippets(requestWithScore);
    ...
    private String generateRefactoringActionHtmlWithSnippets(RefactoringActionReportRequest request) {
        ...
        try {
            ...
            List<HtmlSnippet> snippets = factory.createSnippets(request);
    ...
    /**
     * Generates HTML content for consolidated reports using the snippet-based architecture.
     * Delegates to RiskAnalysisSnippetFactory and ReportAssembler for rich, consistent output.
     */
    private String generateConsolidatedHtml(ConsolidatedReportRequest request) {
        // Build a RiskAnalysisReportRequest from the consolidated request
        RiskAnalysisReportRequest riskRequest = new RiskAnalysisReportRequest(
            request.outputPath(),
            request.projectName(),
            request.reportTitle(),
            request.dependencyGraph(),
        ...
        RiskAnalysisSnippetFactory factory = new RiskAnalysisSnippetFactory();
        List<HtmlSnippet> snippets = factory.createSnippets(riskRequest);

        ReportAssembler assembler = new ReportAssembler();
        return assembler.assembleReport(snippets, request.reportTitle());
    }
}
