...
/**
 * Result of scanning a project for third-party libraries that haven't been migrated.
 */
public class ThirdPartyLibProjectScanResult {
    ...
    public ThirdPartyLibProjectScanResult(String projectPath, 
                                          List<ThirdPartyLibUsage> libraries,
                                          String buildFile) {
        this.projectPath = projectPath;
    ...
    public List<ThirdPartyLibUsage> getLibraries() {
        return libraries;
    }
    ...
    public RiskLevel getRiskLevel() {
        if (libraries.isEmpty()) return RiskLevel.NONE;
        
        long highComplexity = libraries.stream()
            .filter(l -> l.getComplexity() == ThirdPartyLibUsage.MigrationComplexity.HIGH)
            .count();
        
        if (highComplexity > 0) return RiskLevel.HIGH;
        if (libraries.size() < 3) return RiskLevel.LOW;
        if (libraries.size() < 10) return RiskLevel.MEDIUM;
